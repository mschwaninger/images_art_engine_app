...
const saveImage = (_editionCount) => {
  fs.writeFileSync(
    `${buildDir}/${"Capricorn_"}${("0000" + _editionCount).slice(-4)}.png`,
...
const addMetadata = (_dna, _edition) => {
  ...
  let tempMetadata = {
    dna: _dna.join(""),
    name: `Capricorn_#${("0000" + _edition).slice(-4)}`,
    description: description,
    image: `${baseUri}/Capricorn_${("0000" + _edition).slice(-4)}.png`,
    edition: ("0000" + _edition).slice(-4),
    date: dateTime,
    attributes: attributesList,
    compiler: "Capricorn Art Engine",
  };
...
const addAttributes = (_element) => {
  ...
  let nameValue =  selectedElement.name.replace("Cap_", "");
  if (selectedElement.name.includes("Empty"))
  {
    nameValue = "Nothing";
  }
  attributesList.push({
...
const saveMetaDataSingleFile = (_editionCount) => {
  fs.writeFileSync(
    `${buildDir}/${"Capricorn_"}${_editionCount}.json`,
